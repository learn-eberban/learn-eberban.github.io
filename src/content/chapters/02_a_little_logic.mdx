...
<Latex block formula="\texttt{NEITHER\_OPEN\_ON}(s,x,y): \texttt{NEITHER\_WHEN}(\texttt{IS\_OPEN}(s), \allowbreak \texttt{IS\_TODAY}(x), \texttt{IS\_TODAY}(y)))" />
<Latex block formula="\exists d \ \lnot \texttt{IS\_SUNDAY}(d), \exists s \ \allowbreak \texttt{NEITHER\_OPEN\_ON}(s,d,sunday)" />
