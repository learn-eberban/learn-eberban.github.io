...
These existential atom arguments are bound to atom places. With each binding,
the meaning of an existential atom argument is narrowed by quantity and type.

<Info type="examples">
- <DictEntry client:load id="vgeqteao1c" /> {/* mian */}
- <DictEntry client:load id="mu4oshpucp" /> {/* miun */}
- <DictEntry client:load id="fdju08ghm9" /> {/* zun */}
- pa miun zun mian
...
Propositions chained to properties == arity mismatch:
Propositions expect an arity of zero, while properties are unary. 

Properties chained to propositons if atom place unsused

\[E is hungry\] and \[it is evening\]. It ends up being equivalent for these two words. In any order.
